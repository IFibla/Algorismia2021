...
void RandomSort(vector<set<int>> &v) {
    srand((unsigned int)time(NULL));
    int ran = rand();
    Random r(ran);
    vector<int> positions = r.generate_array(v.size());
...
int main( int argc, char **argv ) {
    ...
    for (int na = 0; na < n_apps; ++na) {
        ...
        vector<set<int>> neighborsCopy = neighbors;
        set<int> solutionVertexs;
        RandomSort(neighborsCopy);
        greedy(neighborsCopy, solutionVertexs);
        ...
        for (int vertex : solutionVertexs) {
            for (int neighbor : neighbors[vertex]) if (solutionVertexs.find(neighbor) == solutionVertexs.end()) neighborsSize[vertex]--;
        }
        ...
        while (not end) {
            ...
            if (foundSuccessor) {
                ...
                ct = timer.elapsed_time(Timer::VIRTUAL);
                
                total_time = 0;
                for (double time : times) total_time += time;
                total_time += ct;
                if (total_time >= time_limit) end = true;
            }
            else end = true;
        ...
        if (total_time >= time_limit) break;
